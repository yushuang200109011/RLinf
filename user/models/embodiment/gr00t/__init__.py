...
def get_model(cfg: DictConfig, torch_dtype=torch.bfloat16):
    ...
    Patcher.add_patch(
        ...
        "user.models.embodiment.gr00t.embodiment_tags.EmbodimentTag",
    )
    Patcher.add_patch(
        "gr00t.data.embodiment_tags.EMBODIMENT_TAG_MAPPING",
        "user.models.embodiment.gr00t.embodiment_tags.EMBODIMENT_TAG_MAPPING",
    )
    Patcher.apply()

    from gr00t.experiment.data_config import load_data_config

    from user.models.embodiment.gr00t.gr00t_action_model import (
        GR00T_N1_5_ForRLActionPrediction,
    )
    from user.models.embodiment.gr00t.utils import replace_dropout_with_identity

    if cfg.embodiment_tag == "libero_franka" or cfg.embodiment_tag == "isaaclab_franka":
        data_config = load_data_config(
            "user.models.embodiment.gr00t.modality_config:LiberoFrankaDataConfig"
        )
    elif cfg.embodiment_tag == "maniskill_widowx":
        data_config = load_data_config(
            "user.models.embodiment.gr00t.modality_config:ManiskillWidowXDataConfig"
        )
    else:
