...
class ClusterConfig:
    ...
    For component placement format, refer to `user.scheduler.placement.component_placement`. Here is the detailed specification of the node group configuration.
    ...
    cluster:
      ...
      node_groups:
        ...
        2. node_ranks: A list or range expression of node ranks (integers) that belong to this node group. Node ranks are zero-indexed ranks specified via USER_NODE_RANK environment variable before `ray start` on each node.
