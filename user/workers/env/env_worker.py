...
from user.data.embodied_io_struct import EnvOutput
from user.envs import get_env_cls
from user.envs.action_utils import prepare_actions
from user.envs.env_manager import EnvManager
from user.scheduler import Channel, Cluster, Worker
...
class EnvWorker(Worker):
    ...
    def env_interact_step(
        ...
        t1 = time.time()
        with open(f"/mnt/USER/env_chunk_step.txt", "a") as f:
            f.write(f"{t1}, {t1-t0}\n")
    ...
    def recv_chunk_actions_0(self, input_channel: Channel, mode="train") -> np.ndarray:
        ...
        for gather_id in range(self.gather_num):
            ...
            t = time.time()
            with open(f"/mnt/USER/recv_chunk_actions_0.txt", "a") as f:
    ...
    def recv_chunk_actions_1(self, input_channel: Channel, mode="train") -> np.ndarray:
        ...
        for gather_id in range(self.gather_num):
            ...
            def _callback():
                ...
                with open("/mnt/USER/recv_chunk_actions_1.txt", "a") as f:
