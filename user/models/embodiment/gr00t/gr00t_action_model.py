...
from user.models.embodiment.gr00t.simulation_io import (
    ACTION_CONVERSION,
    OBS_CONVERSION,
)
from user.models.embodiment.gr00t.utils import (
    squeeze_dict_values,
    unsqueeze_dict_values,
)
from user.models.embodiment.modules.explore_noise_net import ExploreNoiseNet
...
class GR00T_N1_5_ForRLActionPrediction(GR00T_N1_5, BasePolicy):
    ...
    def _get_rl_action(
        ...
        backbone_outputs = self.backbone(backbone_inputs)
        action_head_outputs, user_outputs = self.action_head.get_rl_action(
            backbone_outputs, action_inputs, mode=mode
        )
        actions = user_outputs["actions"]
        self.validate_data(action_head_outputs, backbone_outputs, is_training=False)
        actions = actions.float()

        forward_inputs = {
            "chains": user_outputs["chains"],
            "denoise_inds": user_outputs["denoise_inds"],
            **normalized_input,
        }
        ...
        result = {
            "prev_logprobs": user_outputs["prev_logprobs"],
            "prev_values": user_outputs["prev_values"],
