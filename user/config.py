...
from user.envs import SupportedEnvType
from user.scheduler.cluster import Cluster
from user.utils.placement import (
...
def validate_megatron_cfg(cfg: DictConfig) -> DictConfig:
    ...
    with open_dict(cfg):
        ...
        # In USER, padded_vocab_size is set to hf_config.vocab_size, so make_vocab_size_divisible_by=1
...
def build_transformer_config(cfg) -> "TransformerConfig":
    ...
    For attributes in the USER model config that are the same
    as the megatron core TransformerConfig, we will use the value from the USER model config.
    For attributes in TransformerConfig that are not in the USER model config, we add custom logic.
    """
    from megatron.core.transformer.transformer_config import TransformerConfig
    ...
    # any configs that are not in the USER model config will be added here
    config_mapping = {
...
def _build_model_parallel_config(cfg: DictConfig) -> "ModelParallelConfig":
    """
    For attributes in the USER model config that are the same as the
    megatron core ModelParallelConfig we will use the value from the USER config.
    For attributes in ModelParallelConfig that are not in the USER model config, we add custom logic.
    """
    from megatron.core.model_parallel_config import ModelParallelConfig
